...
async function getOrCreateCounter() {
  ...
  const rows = await backend.entities.usage_counter.filter({ month });
  if (rows && rows.length > 0) return rows[0];
  return backend.entities.usage_counter.create({
    month,
    documents_created: 0,
    tts_seconds_used: 0,
  });
...
export async function getMonthlyUsage(): Promise<{
  ...
  try {
    const rows = await backend.entities.usage_counter.filter({ month: currentMonth() });
...
export async function recordDocumentCreated(): Promise<void> {
  try {
    ...
    await backend.entities.usage_counter.update(counter.id, {
