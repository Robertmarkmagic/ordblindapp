...
export function AudioBar({
  ...
  const activeVoiceLabel = hdVoiceId
    ? hdVoices.find((v) => v.id === hdVoiceId)?.label ?? "Natural voice"
    : "Standard voice";
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border/70 bg-background/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-3xl items-center gap-2 px-4 py-3 sm:gap-3 sm:px-6">
        ...
        <Button
          ...
          onClick={onToggle}
          disabled={isLoading}
          aria-label={isPlaying ? "Pause" : isLoading ? "Preparing audio" : "Play"}
        >
          {isLoading ? (
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              aria-label={`Voice, currently ${activeVoiceLabel}`}
            >
              {hdVoiceId ? (
          ...
          <DropdownMenuContent align="end" className="w-64 rounded-2xl">
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">Free · works offline</span>
              </div>
      ...
      {engine === "hd" && (
        <p className="pb-2 text-center text-xs text-muted-foreground">
          Natural voice · highlight follows along
        </p>
      )}
      ...
      <div className="pb-2 text-center">
        <button
          ...
          aria-label="Troubleshoot: no sound"
        >
